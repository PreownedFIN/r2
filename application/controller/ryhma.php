 ...
 *
 */
class Ryhma extends Controller
{
    /**
     * PAGE: index
    ...
    {
		echo "olet käyttäjäcontrollerin index-funktiossa";
	...
	// UC 5: haetaan tietokannasta kaikki ryhmät
	public function haeRyhmat()
	{
		//TODO: suojaa tämä jotenkin, että suorat sivupyynnöt eivät onnistu		
		$r_model = $this->lataaModel('ryhmamodel');
		echo $r_model->haeKaikkiRyhmat();
	}
	
	public function haeRyhmanJasenet() {
		$r_model = $this->lataaModel('kayttajaModel');
		echo $r_model->haeRyhmanJasenet();
	}//haeRyhmanJasenet
    ...
    {
		// tarkistetaan ensin, että tänne saapui ryhmänimi ja salasana-parametrit
		if(isset($_POST["rl"]) && isset($_POST["rn"]) && isset($_POST["rs"])) {
		// echo "parametrejä tuli";
			//TODO: tarkista, että annetut tiedot eivät sisällä kiellettyjä merkkejä
			//TODO: muista kryptata salasana ennen tietokantaan tallennusta
			
			// otetaan ryhmämodeli käyttöön ja tehdään tietokantalisäys sen avulla
			// lisätyn ryhmän id tulee paluupostissa modelilta
			$r_model = $this->lataaModel('ryhmamodel');
			$uusi_id = $r_model->lisaa($_POST["rl"], $_POST["rn"], $_POST["rs"]);
